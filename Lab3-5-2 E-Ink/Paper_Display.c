    ...
    for(row=0; row<16; row++)  // 总共128 SOURCE行，每个像素1bit,即 128/8=16 字节
    {
      switch (num)
      {
		...
		case PIC_WHITE:
          ...
          break;
		case 4:
		    SPI4W_WRITEDATA(gImage_aaa[pcnt]);
		    break;
	    case 5:
	            SPI4W_WRITEDATA(gImage_jing[pcnt]);
	            break;
          default:
			 ...
			 )
{
	...
	while(*str)
	{
	   ...
	   for(i=0;i<zimoW;i++)
	   {
			...
			{
			    volatile unsigned int e = (xsize+i)*16+((ysize/8)+j);
			    DisBuffer[e]=zimoBufferTemp[j];
			    SPI4W_WRITEDATA(DisBuffer[e]);
			}
	   }
	   xsize=xsize+zimoW;
	   str++;
