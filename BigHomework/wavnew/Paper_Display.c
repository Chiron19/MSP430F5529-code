...
void DIS_IMG(unsigned char num)
{
  ...
  for(col=0; col<250; col++)   // 总共250 GATE列  // send 128x250bits ram 2D13
  {
    for(row=0; row<16; row++)  // 总共128 SOURCE行，每个像素1bit,即 128/8=16 字节
    {
      ...
      {
      case 0:
                SPI4W_WRITEDATA(gImage_dora[pcnt]);
                break;
      case 1:
                SPI4W_WRITEDATA(gImage_1a[pcnt]);
                break;
      case 2:
                SPI4W_WRITEDATA(gImage_2a[pcnt]);
                break;
      case 3:
                SPI4W_WRITEDATA(gImage_3a[pcnt]);
                break;
      case 4:
                SPI4W_WRITEDATA(gImage_end[pcnt]);
                break;
		case PIC_WHITE:
