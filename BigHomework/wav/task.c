...
void IOInit()
{
	...
	P1OUT |= BIT4;
	P2DIR |= BIT0+BIT1;              			// 设置P8.1口为输出模式
	P2OUT |= BIT0+BIT1;							// 选中P8.1为输出方式
	P2REN |= BIT1;							// P8.1使能
...
//	P1DIR &= ~BIT3;                       // 设置P8.1口为输出模式
//    P1OUT |= BIT3;                        // 选中P8.1为输出方式
//    P1REN = BIT3+BIT2;
}
void ScanKey(void)
{
...
{
    __disable_interrupt();
    valueCCR2 = valueCCR4 = 0;
    i2 = i4 = 0;
    times_TACCR2 = times_TACCR4 = 1;
    __enable_interrupt();
}
/*手势判断*/
...
{
    __disable_interrupt();
    if ((valueCCR4 > cmpLow && valueCCR2 > cmpLow) && (valueCCR4 < cmpHigh*0.8 && valueCCR2 < cmpHigh*0.8)) {
        modeSel[modeCnt] = 1;
//        resetLED();
...
//        KeyFlag.S1 ^= 1;
    ...
    if ((valueCCR2 > cmpLow && valueCCR4 <= cmpLow) && (valueCCR4 < cmpHigh && valueCCR2 < cmpHigh)) {
        modeSel[modeCnt] = 2;
//        resetLED();
//        runLED0(valueCCR3);
//        __delay_cycles(signalDelay);
...
//        KeyFlag.S2 = 1;
    ...
    if ((valueCCR4 > cmpLow && valueCCR2 <= cmpLow) && (valueCCR4 < cmpHigh && valueCCR2 < cmpHigh)) {
        modeSel[modeCnt] = 3;
//        resetLED();
//        runLED1(valueCCR4);
//        __delay_cycles(signalDelay);
